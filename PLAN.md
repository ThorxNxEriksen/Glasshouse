...
- Skills and MCP tools have no dedicated events — they appear as ordinary `PreToolUse`/`PostToolUse` where `tool_name` is the skill name or matches `mcp__<server>__<tool>`.
